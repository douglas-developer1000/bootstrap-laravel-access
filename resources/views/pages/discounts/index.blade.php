@use ('App\Models\Discount')
@use ('App\Models\User')
@use ('App\Facades\DateFormatter')
@use ('App\Libraries\Enums\DiscountTypeEnum')

@push ('styling')
    @vite ([
        'resources/css/pages/generic/index.css',
        'resources/css/pages/generic/table.css',
    ])
@endpush
@push ('ecmascript-bottom')
    @vite ([
        'resources/js/pages/generic/multiselection.ts'
    ])
@endpush

@php
    $trashed = request()->boolean('trashed');
    $subject = $trashed ? 'Descontos removidos' : 'Descontos';
@endphp

<x-layout title="{{ $subject }}">
    <x-packs.header>
        <x-packs.page-heading-row
            :heading="$subject"
            class="page-heading-row-custom"
        >
            <div class="dropdown top-right-item d-flex gap-2">
                @can ('beSuperAdmin', User::class)
                    <form
                        action="{{ route('discounts.flush') }}"
                        method="post"
                    >
                        @csrf
                        <x-atoms.button
                            type="submit"
                            class="btn btn-secondary"
                        >
                            <i class="bi bi-database-fill-up"></i>
                        </x-atoms.button>
                    </form>
                @endcan
                <x-atoms.button
                    class="btn-secondary dropdown-toggle"
                    data-bs-toggle="dropdown"
                    aria-expanded="false"
                >
                    <i class="bi bi-menu-button-wide"></i>
                </x-atoms.button>
                <ul class="dropdown-menu dropdown-menu-end">
                    <li>
                        <x-atoms.button
                            title="Descontos {{ $trashed ? 'ativos' : 'removidos' }}"
                            class="dropdown-item"
                            format="anchor"
                            href="{{
                                route(
                                    'discounts.index',
                                    $trashed ? [] : ['trashed' => 1]
                                )
                            }}"
                        >
                            @if ($trashed)
                                <i class="bi bi-check-lg"></i>
                                Descontos Ativos
                            @else
                                <i class="bi bi-trash"></i>
                                Descontos Removidos
                            @endif
                        </x-atoms.button>
                    </li>
                    @if (!$trashed)
                        @can ('create', Discount::class)
                            <li>
                                <x-atoms.button
                                    class="dropdown-item"
                                    format="anchor"
                                    href="{{ route('discounts.create') }}"
                                >
                                    <i class="bi bi-plus-lg"></i>
                                    <span>Desconto</span>
                                </x-atoms.button>
                            </li>
                        @endcan
                    @endif
                </ul>
            </div>
        </x-packs.page-heading-row>
    </x-packs.header>
    <main class="bg-secondary-subtle list-main">
        <section class="content bg-light">
            <x-molecules.block-error
                :keys="[
                    'remotion', 'remotion.*', 'restoration', 'restoration.*'
                ]"
            />
            <div class="d-flex flex-wrap justify-content-between row-gap-2">
                <x-organisms.filter-form-check
                    key="own"
                    :checked="request()->boolean('own')"
                    class="py-2"
                >
                    Somente meus descontos</x-organisms.filter-form-check
                >
                <div
                    class="d-flex justify-content-end flex-grow-1 column-gap-2"
                >
                    @if ($trashed)
                        <x-organisms.confirm-restore-group-btn
                            :routeParams="['key' => 'restoration', 'discountList' => 'trashed']"
                            route="discounts.group.restore"
                            heading="Restaurar estes descontos?"
                            positive-text="Restaurar descontos"
                            title="Restaurar descontos selecionados"
                        >
                            Isso restaurará os descontos selecionados e suas
                            utilizações relacionadas.
                        </x-organisms.confirm-restore-group-btn>
                    @else
                        <x-organisms.confirm-rm-group-btn
                            :routeParams="['key' => 'remotion', 'discountList' => 'list']"
                            route="discounts.group.destroy"
                            heading="Remover estes descontos?"
                            positive-text="Remover descontos"
                            title="Remover descontos selecionados"
                        >
                            <div class="mb-3">
                                Para cada desconto selecionado:
                            </div>
                            <div class="mb-1">
                                Se ele não possuir utilização, será removido
                                permanentemente.
                            </div>
                            <div>
                                Caso contrário, será removido apenas desta
                                listagem.
                            </div>
                        </x-organisms.confirm-rm-group-btn>
                    @endif
                </div>
            </div>
            <x-molecules.table-index
                :styleRows="[
                    'first' => 'width: 1.75em;',
                    'second' => 'width: 6.5em;',
                ]"
                :qtyBtns="$trashed ? 1 : 2"
            >
                <x-slot:cols>
                    <col
                        class="col-remain-type"
                        style="visibility: visible; width: auto"
                    />
                    <col class="col-remain-created_at" />
                </x-slot:cols>
                <thead>
                    <tr>
                        <th scope="col">
                            <input
                                type="checkbox"
                                class="form-check-input cursor-pointer multiselection-all"
                            />
                        </th>
                        <x-atoms.table-head sort="value">
                            Valor</x-atoms.table-head
                        >
                        <x-atoms.table-head sort="type">
                            Tipo
                        </x-atoms.table-head>
                        <x-atoms.table-head
                            default
                            colRemain
                            sort="created_at"
                        >
                            Criação</x-atoms.table-head
                        >
                        <th
                            scope="col"
                            class="last-thdata"
                        >
                            Ações
                        </th>
                    </tr>
                </thead>
                <tbody>
                    @forelse ($models($list) as $discount)
                        <tr>
                            <td>
                                <input
                                    type="checkbox"
                                    value="{{ $discount->id }}"
                                    class="form-check-input cursor-pointer multiselection-item"
                                    @disabled ($trashed ? !$hasAccess('restore', $discount) : !$hasAccess('delete', $discount))
                                />
                            </td>
                            <td>
                                <div class="text-truncate">
                                    {{
                                        $parseDiscountEnum($discount->type->value, $discount->value)
                                    }}
                                </div>
                            </td>
                            <td>
                                <div class="text-truncate">
                                    {{ $discount->type->toString() }}
                                </div>
                            </td>
                            <td>
                                {{ DateFormatter::formatToDate($discount->created_at) }}
                            </td>
                            <td>
                                <div
                                    class="w-100 d-flex justify-content-center gap-1"
                                >
                                    @if ($trashed)
                                        <x-organisms.confirm-restore-btn
                                            :routeParams="['discountDeleted' => $discount->id]"
                                            route="discounts.restore"
                                            heading="Restaurar este desconto?"
                                            positiveText="Restaurar desconto"
                                            title="Restaurar desconto"
                                            :disabled="!$hasAccess('restore', $discount)"
                                        >
                                            Isso restaurará este desconto e suas
                                            utilizações relacionadas.
                                        </x-organisms.confirm-restore-btn>
                                    @else
                                        <x-atoms.button
                                            format="anchor"
                                            class="btn-secondary"
                                            href="{{ route('discounts.edit', ['discount' => $discount->id]) }}"
                                            :disabled="!$hasAccess('edit', $discount)"
                                        >
                                            <i class="bi bi-wrench"></i>
                                        </x-atoms.button>
                                        <x-organisms.confirm-rm-btn
                                            :routeParams="['discount' => $discount->id]"
                                            route="discounts.destroy"
                                            heading="Remover este desconto?"
                                            positiveText="Remover desconto"
                                            title="Remover desconto"
                                            :disabled="!$hasAccess('delete', $discount)"
                                        >
                                            <div class="mb-1">
                                                Se ele não possuir utilização,
                                                será removido permanentemente.
                                            </div>
                                            <div>
                                                Caso contrário, será removido
                                                apenas desta listagem.
                                            </div>
                                        </x-organisms.confirm-rm-btn>
                                    @endif
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td
                                colspan="5"
                                class="no-values"
                            >
                                Sem descontos para o filtro atual
                            </td>
                        </tr>
                    @endforelse
                </tbody>
            </x-molecules.table-index>
            <x-molecules.root-pagination :paginator="$list" />
        </section>
        <x-packs.toast />
    </main>
</x-layout>
